...
class Board:
    def __init__(self, hid=False, size=9):
        ...
        self.count = 0  # счетчик уничтоженных кораблей (в модуле "shot")

        self.field = [["~"] * size for _ in range(size)]

        self.busy = []  # занятые точки (кораблем или пораженная точка)
    ...
    def begin(self):
        self.busy = []  # обнуленный список, чтобы хранить точки стрельбы
...
class Game:
    ...
    def try_board(self):
        lens = [4, 3, 3, 2, 2, 2, 1, 1, 1, 1]  # длины кораблей
    ...
    def merge_boards(first, second):
        ...
        for i in range(len(first_lines)):
            result_line = f"{first_lines[i]:40}   ||   {second_lines[i]} \n"
            result += result_line
        return result
